...
def _write_output(
    ...
    datasheet_path.write_text(
        f"# Personalised Memory Benchmark (PMB) v0 — generated corpus\n\n"
        f"## Overview\n"
        f"Generated by `scripts/build_pmb.py`. This is a *generated corpus*, and its role depends "
        f"on where it was written: the `pmb_*` directories are the **evaluation benchmark**; the "
        f"`sft_personas_*` directories are disjoint **training-persona** sets and must not be used "
        f"for evaluation or reported as benchmark results.\n\n"
        ...
        f"## Limitations\n\n"
        f"{limitations}"
        f"- The `acceptable_alternatives` field is present on every probe but is "
        f"currently unpopulated, so exact-match (`pra_strict`) scoring is ~0 by "
        f"construction; judge-scored `pra_lenient` is the metric to report.\n",
        encoding="utf-8",
        newline="\n",
    )


def _encode_for_hash(text: str) -> bytes:
    """Encode to UTF-8 with newlines normalised to LF.

    Without this, the digest depends on the platform's line-ending convention: this repo sets
    `core.autocrlf=true`, so a corpus generated on Windows is CRLF and the same corpus generated
    on Linux is LF, giving different hashes for byte-identical content. A committed `hash.txt` also
    stops verifying after a fresh clone on the other platform. Normalising makes the hash a
...
def _compute_hash(out_dir: Path) -> str:
    ...
    for root, _dirs, files in sorted(os.walk(out_dir)):
        for fname in sorted(files):
            ...
            hasher.update(_encode_for_hash(rel.as_posix()))
            hasher.update(_encode_for_hash(fpath.read_text(encoding="utf-8")))
